...
| H3 · INSERT pure-win waste | ✅ closed (residual inherent) | **Plain-rowid max cache ✅** (`RelationState.maxRowidCache`: skips the per-insert `move(.last)` O(depth) descent for ascending inserts — the W2 probe; rollback-safe via `TxnRestorePoint`, invalidated on delete/drop, kept consistent with explicit rowids, not serialized; 339 tests incl. SQL INSERT/upsert + group-commit + crash-injection + the SQLite rowid differential). Bench 200k ~132→135k/s — a real waste-clear but **small**: the 4 COW B+tree puts (table + 3 indexes) dominate (finding #5), so **beating SQLite on INSERT needs `appendCursor` (H5, crash-critical)** — the table-tree sequential-append fast path. **Hoisted insert path ✅** (`Insert.hoisted`, strategy-beside, opt-in; R1): (a) caches the owned-index roster per (txn, tableId) — the index-name set is invariant within an INSERT statement, so the per-row filter+sort+allocation is paid once; invalidated by index-set DDL (`createIndex`/`dropIndex`/`dropTable`) + request-scope rollback; (b) drops ctx's alias to the relational state *past* conflict resolution, so the write loop's table/index handle updates mutate the dictionaries in place instead of COW-copying each on first touch per row — the alias is dropped only AFTER the OR IGNORE/ABORT discard paths so their "leave ctx.relation intact" semantics hold. `hoisted ≡ standard ≡ SQLite` (4 new differential tests: multi-index bulk+REPLACE, OR IGNORE discard preserves in-txn state, DDL-mid-txn roster invalidation, 700-op fuzz; `verifyIntegrity(deep:)` clean). **Bench 200k warm: hoisted ~141–144k/s vs standard ~142–143k/s — at parity within run-to-run noise (slight positive bias), no regression**, honestly reconfirming finding #5: the 3 secondary-index COW B+tree puts (per-index `Node.search` descent + `leafInsert`) dominate and are shared-algorithm with SQLite. Default stays `.standard`. **Deep profile (R1 layer-3 — `/usr/bin/sample` on the SQL 3-index positional insert, the headline path) — recorded:** writer self-time ≈ ARC (retain/release/bridgeObject/isUniquelyReferenced) **~25%** + inherent B+tree core (`Node.search`/`memcmp`/`memmove`, shared-algorithm with SQLite) **~16%** + schema-dict String-hash/lookup **~10%** + temporary allocation **~10%** + `pwritev`/commit/`fcntl` **~8%** + generic-metadata instantiation **~2%** + key/record encode. **Conclusion: the residual (~0.8× warm) is the diffuse Swift-runtime tax (ARC + alloc + dict + metadata ≈ 47%) over a shared-algorithm B+tree — no single deep lever; addressable only as a broad borrow / zero-copy / de-generic campaign at ~1–2% per slice with rising risk.** The ARC component is the *same* per-row `[Value]`/String row-materialization that taxes JOIN, SEARCH and FTS-MATCH (review 0003 finding #7), so it is attacked with **leverage** by the unifying row path (H6/R3), not INSERT-specific micro-tuning. **R1 closes here:** safe waste cleared (`maxRowidCache` + `hoisted` + `appendCursor`), residual characterized as inherent. |
| H4 · JOIN merge + cost model + semi-join | 🚧 in progress | **Hash semi-join ✅** (`runInnerHashJoin`: for `innerExistenceOnly` + no residual, build a per-key COUNT map instead of materializing inner rows → emit count×; fixes the materialize-everything problem, findings #1/#3; `SQLHashJoinTests` gains a UNIQUE join-key index so the `COUNT(*)` self-join actually drives it; `.hash ≡ .nestedLoop ≡ SQLite`, 339 tests; bench 2k `.hash` join ~2.8 ms — still **above** nested-loop, empirically reconfirming merge is the symmetric-self-join winner). **Merge join existence/COUNT fast path ✅** (`runMergeJoin` — a 2-table INNER existence self-equi-join on a UNIQUE NOT-NULL single-column index → one ordered index pass emitting per entry, no per-outer probe; UNIQUE+NOT-NULL rules out dup-runs/NULL so it is provably `≡` nested-loop; default stays nestedLoop — strategy-beside; `merge ≡ nestedLoop ≡ hash ≡ SQLite`, 339 tests). **Bench 20k: merge join p50 ~1.9–2.1 ms vs SQLite 4.3 ms ≈ 2.1× FASTER; vs nested-loop ~17 ms ≈ 8.4× — the headline JOIN 0.26× loss is a win under `.merge`.** **Cost model `.auto` ✅** (conservative: picks the merge fast path when eligible — unconditionally cheaper here, one index pass vs M probes — else nested-loop; hash is **not** auto-selected pending a build-side estimate since it loses on the symmetric self-join; `auto ≡ nestedLoop ≡ hash ≡ merge ≡ SQLite`, 339 tests; **bench 20k `--join auto` picks merge → p50 ~2.1 ms vs SQLite ~4.5 ms ≈ 2.1× faster**). **Next: default-flip `.auto`** (H8 seven-criteria gate → the out-of-box JOIN beats SQLite) + **R2: general 2-table INNER existence merge ✅** — `BoundSelect.mergePlan` (binder, strategy-independent metadata: both join-key columns have a UNIQUE NOT-NULL single-col index, same collation) + `context.mergeIndexes` (resolved in Statement) drive a **two-cursor lock-step** `runMergeJoin` (key-prefix compare, A4; the self-join is subsumed = both cursors over the one index). `merge ≡ nestedLoop ≡ hash ≡ auto ≡ SQLite`, **345 tests** incl. a 2-table key-intersection differential; release build strict-MS-clean. Remaining (R2 follow-on): dup-run cross-product, NULL-run skip, LEFT, and projecting (non-existence) merges. |
| H5 · INSERT appendCursor | ✅ done (opt-in) | `BTree.appendMax` warm rightmost-leaf append behind `Insert.appendCursor`: in-place append routed through `ctx.shadow` (undo-recorded → group-commit-rollback-safe), per-entry `rootPage` staleness guard, cache cleared on rollback; any split / non-ascending / stale / explicit-rowid case falls through to the proven `put`, which refreshes the cache via a rightmost descent. **Extensively tested** (`SQLAppendCursorTests`, 5 tests; 344 total green): differential `appendCursor ≡ standard ≡ SQLite` (splits/deletes/explicit/OR-REPLACE) + `verifyIntegrity(deep:)`, single-txn rollback undo, **group-commit rollback isolation** (the HIGH-risk shared-ctx in-place-append undo), crash-recovery reopen, seeded fuzz. **Honest bench (`--insert appendCursor`, 200k/3-index): ~136 vs ~135k/s standard (~1%, within noise)** — it optimizes only the table tree (1 of 4 puts); the **3 secondary-index COW puts dominate** (finding #5), so the 3-index INSERT stays ~0.74× vs SQLite. It helps table-tree-dominated (few-index) inserts; closing the 3-index INSERT gap needs index-maintenance optimization (a separate lever). Default stays `.standard` (flip via H8 seven-criteria gate). · **R1 profile** (`/usr/bin/sample`, 1M-row insert, self-time): insert is **writer-thread-bound**; the writer's per-row compute is dominated by **`Node.search` (4 B-tree descents/row) + `memcmp`** (both shared with SQLite), then a large **ARC/exclusivity cluster** (`swift_release`/`isUniquelyReferenced`/`beginAccess`/`retain`/`bridgeObject*` ~600+ samples — the Swift overhead SQLite's C avoids), plus a smaller per-row `RelationState` copy (`IndexRecord` copy + dict CoW + array growth). **The gap is inherent descent + Swift ARC/exclusivity.** R1 levers: (a) `hoisted` (kill the per-row state copy + index-list rebuild + scratch exclusivity), then (b) the deep layer — reduce ARC/exclusivity in the universal `Node.search`/`leafInsert` primitives (high blast radius). · **Index-list dedup ✅** (compute the owned-index set once/row, not twice — the unique-conflict scan + the write loop now share one stable `ownIndexNames`; identical results, 344 tests; bench ~135→~137k/s, ~1–3%). **The safe relational-layer wins (rowid cache + appendCursor + dedup) are now exhausted at ~1–3% each; INSERT stays ~0.77× — the residual is the inherent B-tree descent (shared with SQLite) + Swift ARC/exclusivity in the universal primitives.** Remaining R1 (delicate/high-blast-radius): the in-place state-copy reorder (conflict-before-allocate) + the `Node.search`/`leafInsert` ARC reduction. |
| H6 · evaluator + row path | 🚧 in progress (compiled is default) | wider `CompiledEval` + VDBE + W5. **R3 step-1 ✅ — widened `CompiledEval` + flipped the default to `.compiledClosures`.** Added `.like`, `.inList` (baked subject/item affinity + collation + the `lhs` carry across items, mirroring `SQLEval.inList` exactly) and `.function` (a scalar call participates in an otherwise-compiled expression; its args still tree-walk through the same `env`) to the bind-time compiler; the rest (correlated `.column`, `.scalarSubquery`, `.inJSONEach`, `.aggregateResult`) still fall back per-subexpression, so the path stays safe-by-construction. `compiled ≡ treeWalk ≡ SQLite` across the evaluator strategy matrix (now also LIKE / IN-list / scalar-function / NULL-3VL / NOCASE; 349 tests). **Default flip (H8 seven-criteria):** accuracy ✅ (matrix) · perf ✅ (clean eval A/B, compiled vs treeWalk: key-select p50 4.3 vs 4.8 µs / p99 22 vs 42 µs, search p50 4348 vs 4539 µs / p99.9 6.4 vs 8.9 ms, distinct ≈parity — compiled is at-or-better on p50 with tighter tails, no regression; the per-row eval is a small, access-path-bound slice so the margins are modest) · concurrency/parallelism/integrity ✅ (TSan-clean) (read-path, per-execution thunks, no shared mutable state — TSan-clean). `ADSQLBench` defaults realigned to the shipped library defaults (`compiledClosures` + `auto`). **Remaining (deferred — evidence-led):** the VDBE register machine is the only lever that cuts per-row eval cost further, but the read paths are **access-path-bound** (eval is a small, Amdahl-bounded slice — A/B shows ≈parity), so it is held as a high-cost spike rather than a multi-week build; plus the per-row `[Value]` row-materialization (W5). |
