...
| H2 · structure + safe-type fixes | 🚧 in progress | `enum Binder` extracted `Plan.swift`→`Binder.swift` ✅ (975→211+764, pure motion, 339 tests green). Remaining: Executor/Parser/Writer/Relation/DML splits + `Postings` force-unwrap. (`FTSIndex` redundant-`unsafe` finding **dropped** — F6k/F6n made it a required zero-copy raw-buffer read; `Trigram:60` is range-guarded.) |
