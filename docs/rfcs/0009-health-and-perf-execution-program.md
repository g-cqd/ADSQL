...
| H4 · JOIN merge + cost model + semi-join | 🚧 in progress | **Hash semi-join ✅** (`runInnerHashJoin`: for `innerExistenceOnly` + no residual, build a per-key COUNT map instead of materializing inner rows → emit count×; fixes the materialize-everything problem, findings #1/#3; `SQLHashJoinTests` gains a UNIQUE join-key index so the `COUNT(*)` self-join actually drives it; `.hash ≡ .nestedLoop ≡ SQLite`, 339 tests; bench 2k `.hash` join ~2.8 ms — still **above** nested-loop, empirically reconfirming merge is the symmetric-self-join winner). **Next: merge join + cost model** — a `.merge` branch in `forEachFilteredRow` at the `.hash` dispatch seam (`Executor.swift:725-733`) → new `MergeJoin.swift`, **eligible-or-fall-through** (mirrors `runInnerHashJoin`; default stays nestedLoop — strategy-beside). Eligibility: 2-table INNER equi-join, both sides sorted on the join key (same collation), extracted via the `hashEquiKey` pattern; lock-step byte-compare on the key-prefix (strip the 8-byte rowid suffix, A4); **existence/COUNT(\*) fast path** multiplies dup-run lengths (the bench winner); LEFT/non-unique/non-equi fall back. Gate: `merge ≡ nestedLoop ≡ hash ≡ SQLite` differential + StrategyBench. Focused multi-commit effort. |
