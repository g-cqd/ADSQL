...
struct SQLEvalDifferentialTests {
    ...
    func randomExpressions(seed: UInt64) throws {
        ...
        func expression(_ depth: Int) -> String {
            ...
            case 4:
                return
                    "(\(expression(depth - 1)) \(["=", "!=", "<", "<=", ">", ">="][Int(rng.next() % 6)]) \(expression(depth - 1)))"
            ...
            for b in samples {
                ...
                ab.withUnsafeBytes { ap in
                    bb.withUnsafeBytes { bp in
                        #expect(
                            sign(SQLCompare.compareUTF8(ap, bp)) == sign(SQLCompare.compareUTF8(a, b)),
                            "BINARY mismatch \(a) vs \(b)")
                        #expect(
                            sign(SQLCompare.compareUTF8NoCase(ap, bp)) == sign(SQLCompare.compareUTF8NoCase(a, b)),
