...
struct SQLParserStatementTests {
    ...
    @Test func insertVariants() throws {
        guard
            case .insert(let plain) = try parse(
                "INSERT INTO kv (k, v) VALUES (?, ?), ($a, $b)")
        else {
            Issue.record("not insert")
            return
        }
        #expect(plain.columns == ["k", "v"])
        if case .values(let rows) = plain.source {
            #expect(rows.count == 2)
        } else {
        ...
        guard
            case .insert(let orReplace) = try parse(
        ...
        guard
            case .insert(let upsert) = try parse(
                """
        ...
        guard
            case .insert(let doNothing) = try parse(
    ...
    @Test func tableLevelPrimaryKey() throws {
        ...
        guard
            case .createTable(let ct) = try parse(
        ...
        guard
            case .createTable(let ct2) = try parse(
    ...
    @Test func indexAndDropAndTxn() throws {
        guard
            case .createIndex(let ci) = try parse(
                "CREATE UNIQUE INDEX IF NOT EXISTS u_docs_key ON documents (key)")
        else {
            Issue.record("not createIndex")
            return
        }
...
struct SQLParserErrorTests {
    ...
    @Test func fuzzTruncateAndSplice() {
        let corpus =
            SQLParserStatementTests().corpusForFuzz
