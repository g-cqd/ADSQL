    ...
    @Test func createPersistsAcrossReopen() throws {
        ...
        do {
            ...
            try db.writeSync { (txn) throws(DBError) in
                ...
                try txn.createIndex(
                    IndexDefinition("idx_documents_key", on: "documents", columns: ["key"], unique: true))
        ...
        #expect(schema.tables["documents"] == documentsTable())
        #expect(
            schema.indexes(on: "documents").map(\.name)
            ...
            try db.writeSync { (txn) throws(DBError) in
                try txn.createTable(
                    TableDefinition(
                        ...
                        foreignKeys: [
                            ForeignKey(childColumns: ["doc_id"], parentTable: "missing_parent", onDelete: .cascade)
                        ]))
            }
        }
        ...
        try db.writeSync { (txn) throws(DBError) in
            try txn.createTable(
                TableDefinition(
                    ...
                    foreignKeys: [
                        ForeignKey(childColumns: ["document_id"], parentTable: "documents", onDelete: .cascade)
                    ]))
        }
