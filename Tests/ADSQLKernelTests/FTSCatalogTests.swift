...
struct FTSCatalogTests {
    @Test func parsesSelfContainedAndExternalShapes() throws {
        ...
        guard
            case .createVirtualTable(let docs) = try parse(
                """
                CREATE VIRTUAL TABLE documents_fts USING fts5(
                  title, abstract, declaration, headings, key, tokenize='porter unicode61')
                """)
        else {
            Issue.record("not createVirtualTable")
            return
        }
        ...
        guard
            case .createVirtualTable(let trig) = try parse(
                """
    ...
    @Test func parsesContentlessAndPrefixShapes() throws {
        ...
        guard
            case .createVirtualTable(let body) = try parse(
                """
        ...
        guard
            case .createVirtualTable(let sym) = try parse(
                """
