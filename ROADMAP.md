...
| **apple-docs `/search`** (8-way) | **⚠️ UNVERIFIED** (contested) | A **2026-06-15 re-run did NOT reproduce** the prior "ADSQL wins 223 vs 101" headline: on the on-disk **~0.5 GB** denorm corpus (never the "4 GB" claimed — that was apple-docs' *production* size) SQLite **won at every thread count** (8-way ~962 vs ~195 req/s; single-thread ~5.8× slower, 25.5 vs 4.4 ms). BUT two caveats stop this flipping to "SQLite wins": the sweep's req/s are internally inconsistent (unreliable accounting), and ~0.5 GB is RAM-resident so SQLite never hits the memory-bandwidth ceiling that is the *entire* premise of the thesis — the wrong regime. Needs a fixed req/s metric **and** a genuine ≥4 GB working set to settle. Single-thread profile is solid: cost = per-match `documents` SEEK + column decode + bm25 score-all; **A5 refuted**, compiled-join no-win. See **RFC 0010 §6**. |
