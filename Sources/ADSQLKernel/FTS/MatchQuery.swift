...
private enum MatchLexer {
  ...
  static func tokenize(_ bytes: [UInt8]) -> [MatchToken] {
    ...
    while index < bytes.count {
      ...
      case 0x20, 0x09, 0x0A, 0x0D:
        index += 1
      case UInt8(ascii: "("): tokens.append(.lparen); index += 1
      case UInt8(ascii: ")"): tokens.append(.rparen); index += 1
      case UInt8(ascii: "{"): tokens.append(.lbrace); index += 1
      case UInt8(ascii: "}"): tokens.append(.rbrace); index += 1
      case UInt8(ascii: ":"): tokens.append(.colon); index += 1
      case UInt8(ascii: "*"): tokens.append(.star); index += 1
      case UInt8(ascii: "\""):
        index += 1
        let start = index
