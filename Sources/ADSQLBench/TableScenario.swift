...
enum TableScenario {
  ...
  static func runADSQL(path: String, rows: Int, config: BenchConfig) throws {
    ...
    print("  [adsql] index scan      \(scanned) rows in \(scanElapsed / 1_000_000) ms (\(formatRate(scanned, scanElapsed)))")
  }
  ...
  static func runSQLite(path: String, rows: Int, config: BenchConfig) throws {
    ...
    let scanElapsed = nowNanos() - scanStart
    print("  [sqlite] index scan      \(scanned) rows in \(scanElapsed / 1_000_000) ms (\(formatRate(scanned, scanElapsed)))")
  }
}
